...
/*
static paddr_t alloc_pages(uint32_t n) {
...
static void map_page(uint32_t* table1, uint32_t vaddr, paddr_t paddr, uint32_t flags) {
...
static struct sbiret sbi_call(long arg0, long arg1, long arg2, long arg3, long arg4,
...
static void read_write_disk(void* buf, unsigned sector, int is_write) {
...
static struct file* fs_lookup(const char* filename) {
    for (int i = 0; i < FILES_MAX; i++) {
        struct file* file = &files[i];
...
/*
__attribute__((naked)) __attribute__((aligned(4))) static void kernel_entry(void) {
...
/*
static struct process* create_process(const void* image, size_t image_size) {
...
static void handle_trap(struct trap_frame* f) {
...
void kernel_main(void) {
    ...
    */
    ...
    // unreachable - prevent unused-function compilation error
    handle_trap(NULL);
}

/*
__attribute__((section(".text.boot"))) __attribute__((naked)) static void boot(void) {
